...
//  PokemonGo
//
//  Created by Mario Fernando Paucar Gutierrez on 5/20/19.
...
class ViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if contActualizaciones < 1 {
            let region = MKCoordinateRegion(center: ubicacion.location!.coordinate, latitudinalMeters: 1000, longitudinalMeters: 20000)
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        ...
        let region = MKCoordinateRegion(center: view.annotation!.coordinate, latitudinalMeters: 200, longitudinalMeters: 300)
        mapView.setRegion(region, animated: true)
        ...
        Timer.scheduledTimer(withTimeInterval: 1, repeats: false, block:  { (timer) in
            if let coord = self.ubicacion.location?.coordinate {
                ...
                if mapView.visibleMapRect.contains(MKMapPoint(coord)){
                    print("Puede atrapar el pokemón")
                    ...
                    print("No puede atrapar el pokemón")
